...
"""
        ...
        for region in region_rankings:
            content += f"| {region['rank']} | {region['region']} | {region['school_num']} | {region['vacancy']} | {region['applied']} | {region['remaining']} | {region['characteristic_analysis']} |\n"
...
"""
        ...
        for i, region in enumerate(region_rankings[:5], 1):
            content += f"{i}. **{region['region']}** - 超额率: {region['excess_rate']:.3f}\n"
